...
for k in range(len(raw_images)):
    ...
    for i in range(10): # for each training slice, we only consider 10 testing style
        ...
        if mode == 'train':
            ...
            image_1_name = 'testing_axial_crop_pat'+str(sample_no)+'_v'+str(view_index)+'_'+str(mapped_x)+'.jpg'
        if mode == 'test':
            image_1_name = 'training_axial_crop_pat'+str(sample_no)+'_v'+str(view_index)+'_'+str(mapped_x)+'.jpg'
        image_1 = os.path.join(raw_image_dir, image_1_name) 
        image_2_styled_name = sample_name.split('.')[0]+'_'+str(sample_no)+'.jpg'
        image_2_styled = os.path.join(output_dir, image_2_styled_name)
